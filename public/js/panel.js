...
function actualizaPanel(){
    ...
    $.get(url+"api/report"+(adm?(current_search===-2?"/all":"/"+current_search):"")+"/"+parseInt(datefrom.datetimepicker('viewDate')._d.getTime()/1000)+"/"+parseInt(dateto.datetimepicker('viewDate')._d.getTime()/1000),function (data) {
        if (data.length>0){
            var result=null;
            try{
            ...
            mindateweek.setDate(mindateweek.getDate()-mindateweek.getDay()+1);
            ...
            if (result.status === "ok"){
                ...
                result.data.forEach(function(row){
                    ...
                    if (sdate.getDate()===new Date().getDate() && sdate.getMonth() === new Date().getMonth() && sdate.getFullYear() === new Date().getFullYear()){
                        todaySeconds+=totalseconds;
                    }
                    if (sdate.getMonth() === new Date(datefrom.datetimepicker('viewDate')._d).getMonth() && sdate.getFullYear() === new Date(datefrom.datetimepicker('viewDate')._d).getFullYear()){
            ...
            users.forEach(function (user) {
                if (current_search===-2 || user.id === current_search){
                    var date = setTimeZero(new Date(datefrom.datetimepicker('viewDate')._d));
                    var todate = setTimeZero(new Date(dateto.datetimepicker('viewDate')._d));
                    do{
                        if (date.getDay() !== 0 && date.getDay() !== 6){
                            expected_hours+=user.mins; //TODO: sacar horas jornada usuario
            ...
            tmp_dateWeek.setDate(tmp_dateWeek.getDate()+4);
...
function registerDay(e,full){
    ...
    $.post(url+path, full?{date: date,from_hour: from_hour,to_hour: to_hour, breaktime: breaktime, comment: comment,_token: csrf}
        :
...
$(document).ready(function(){
    ...
    btn_export.on('click',function () {
        ...
        table.DataTable().rows({ search: 'applied' }).every( function ( rowIdx, tableLoop, rowLoop ) {
           var d = this.data();
           doc+=_c(d[1])+_c(d[2])+_c(d[3])+_c(d[4])+_c($.parseHTML(d[5])[0].innerHTML)+_c(d[6])+_c(d[7])+_c(d[8])+_c(d[9])+"\r\n";
        });
        var data = new Blob([doc], {type: 'text/csv'});

        var dt = new Date();
        var fname="Export_"+dt.getFullYear()+"_"+(dt.getMonth()+1)+"_"+dt.getDate()+".csv";
    ...
    register_date.off('keypress').keypress(function(e){ return false; });

    datefrom.datetimepicker({
        format: 'L',
        defaultDate: new Date().setDate(1),
        locale: 'es',
        maxDate: new Date().setDate(daysInMonth(new Date().getMonth(),new Date().getFullYear())-1)
    });
    dateto.datetimepicker({
        useCurrent: false,
        format: 'L',
        defaultDate: new Date().setDate(daysInMonth(new Date().getMonth(),new Date().getFullYear())-1),
        locale: 'es',
